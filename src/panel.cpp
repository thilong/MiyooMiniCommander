...
#define PANEL_SIZE         316
#define NAME_SIZE          280
...
void CPanel::render(const bool p_active) const
{
    ...
    for (unsigned int l_i = m_camera; l_i < m_camera + NB_VISIBLE_LINES && l_i < l_nbTotal; ++l_i)
    {
        ...
        if (l_surfaceTmp->w > NAME_SIZE)
        {
            ...
            l_rect.w = NAME_SIZE;
            l_rect.h = l_surfaceTmp->h;
            SDL_utils::applySurface(l_x, l_y + 4, l_surfaceTmp, Globals::g_screen, &l_rect);
        }
        else
        {
            SDL_utils::applySurface(l_x, l_y + 4, l_surfaceTmp, Globals::g_screen);
        }
        SDL_FreeSurface(l_surfaceTmp);
        // Next line
        l_y += LINE_HEIGHT;
